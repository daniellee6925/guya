...
1. **Daemon health check.** `ssh mini cat /Users/guya/constantia/.git/sync-status.json` — heartbeat must be fresh (<5min) and `last_cycle_outcome` should be `ok`, `no-op`, or `pulled` (all healthy). If stale or `error`, daemon needs attention. Guya's session-start hook surfaces this as a `constantia-sync-alert` section automatically; absence of alert ≈ healthy.
...
3. **Week ships — IDs now follow `T`=task / `P`=proposal (2026-05-21 swap).** T-001 (Lina model-as-judge calibration, Pillar 3), T-002 (Lina platform fixes, Pillar 2), T-003 (Boson agent → voice chat integration, Pillar 2 — accepted from proposal P-003). Check `tasks/MANIFEST.md` for live status; the old-scheme IDs (P-001/P-002/T-003) appear in dated logs/evidence with their pre-swap meaning per the CLAUDE.md legend.
...
**The bug.** WORK's self-initiated ticks (morning/midday/evening/plan-daily/plan-weekly/reflect) silently stopped after 2026-06-09 — the channel kept answering live messages, so nothing looked broken. Root cause: recurring ticks live as `messages_in` rows inside a *per-session* `inbound.db`, not in central config. On 2026-06-08 a second WORK session (`sess-…-v0rwsz`) was created while the original (`sess-…-0bacbi`) was still active and ticking — `findSessionForAgent` missed the live session and `resolveSession` minted a duplicate (exact miss-cause not pinned). Live traffic drifted onto the new session; the 6 recurring schedules stayed in the old one. When the old session was later marked `closed`, the host sweep — which only sweeps `status='active'` sessions — stopped firing them. **No host restart was involved** (the host ran continuously since 5/21); the running process created the duplicate itself. Diagnosed from the DB: two sessions for `mg-…y1fe2x`, the `closed` one holding all 6 pending recurring rows with `process_after` frozen at 06-08/09.

**The restore.** Copied the 6 recurring rows from the dead session's `inbound.db` into the live session's, next-fire recomputed to the upcoming occurrence, `series_id` preserved so recurrence keeps chaining, host-even-seq invariant honored (`nextEvenSeq`). Verified live: the 1pm midday tick fired on schedule and the recurrence handler chained it to the next day; all 6 series confirmed live on `v0rwsz`.

**The fix (nanoclaw `6f432a6`).** New `src/session-supersede.ts`: `migrateRecurringSchedule` (copy live pending/paused recurring rows between session inbound.dbs, dedup per `series_id`, retire source rows; **throws rather than closing a source if the target inbound.db is missing** — no silent loss), `supersedeSession` (migrate-then-close), `reconcileChannelSessions` (boot-time heal collapsing duplicate-active channels onto the newest session, loud WARN). Wired into `resolveSession` (supersede any straggler active session on create → one active session per channel, best-effort so it never breaks session creation) and `index.ts` boot. Honors the cross-mount inbound.db invariants (open-write-close, journal DELETE). 11 new tests; full suite 221/221; both Karpathy review passes (deep review caught + fixed the missing-target silent-loss hole). **Deploy:** pushed → Mini `git pull` (`6f432a6`) → `npm run build` → host restart (now PID 71820), clean boot, `reconcileChannelSessions` ran as a no-op (1 active WORK session). Note: this was the first host-code (`src/`) change since the 5/21 build — zero `src/` commits had landed in between, so the restart deployed only this change.
...
- [2026-06-11] **One active session per channel + recurring-schedule carry-over on supersede (nanoclaw `6f432a6`; new `src/session-supersede.ts`).** Recurring ticks live as `messages_in` rows in a *per-session* `inbound.db`, so when a channel's session is replaced (a prompt redeploy spawns a fresh session per ADR-018) the schedule was stranded in the old session and the host sweep — active-sessions-only — silently stopped firing it (cost: WORK ticks dark 06-09→06-11, see Tests & Observations). New invariant: a session may only be retired *through* `supersedeSession`, which first migrates the live recurring schedule to the replacement and refuses to close the source if the carry can't complete (throws → caller leaves it active and logs loudly — no silent loss). Enforced two ways: `resolveSession` supersedes any straggler active session on create, and a boot-time `reconcileChannelSessions` collapses any pre-existing duplicate-active channel onto the newest session with loud WARN logs. Same anti-silent-rot meta-pattern as ADR-011/012/013 — the failure was invisible for two days precisely because the channel still answered live messages. Known limitation: `reconcileChannelSessions` keys channels by (agent_group, messaging_group, thread), so it doesn't model `agent-shared` mode (single-session enforced at create instead) — not reachable for Telos (WORK/LIFE/LEARN are `shared`). **Warrants an ADR-026 entry in `guya/CLAUDE.md` ADR table — amend there separately (follow-up).**

- [2026-06-08] **`constantia-sync` daemon: idle-pull + deploy-key pin (ADR-024 amendment; constantia `scripts/constantia-sync.sh`).** Two fixes after the laptop→Mini sync stalled. (1) **Idle-pull.** `do_cycle` short-circuited to `no-op` the moment `local == last_pushed`, never fetching — so an idle Mini never pulled laptop commits until Telos's own next commit. The idle branch now does a throttled (60s) `git fetch + merge --ff-only`, so the Mini auto-tracks laptop pushes within ~60s regardless of Telos activity. ff-only is provably safe there (`local==last_pushed` ⇒ zero unpushed local commits ⇒ origin equal-or-ahead, never diverged). This **directly revisits the 2026-05-06 "not a periodic pull-cron" note below**: that footgun was *container-side* bind-mount corruption, which ADR-024 already eliminated by moving git to the host's native APFS — host-side ff-only is safe, and we chose B (fold into the one daemon) over a separate puller to avoid a two-process `index.lock` race. (2) **Deploy-key pinned.** Daemon now `export`s `GIT_SSH_COMMAND` with `-i /Users/guya/.config/nanoclaw/constantia-deploy-key -o IdentitiesOnly=yes`; the repo's `core.sshCommand` points at the *container* path (nonexistent on host) and git was silently falling back to the host default identity (`guyacode`, not the scoped `daniellee6925/constantia` deploy key). **Trigger:** first-ever task-file merge conflict — Telos graded T-006 (`status: graded`, grade A) on the Mini from a stale base while the laptop set `status: complete`; same frontmatter line, unmergeable, daemon parked on `conflict`. Resolved by pause→rebase→merge keeping both sides→regen MANIFEST→push→restart; idle-pull is the structural prevention (Telos now grades on top of the latest). Verified live: idle Mini pulled marker commit `bb0a420` on its own (`outcome: "pulled"`, 0/0). Both review passes + `bash -n` clean. **Not yet reflected in the ADR-024 doc / `guya/CLAUDE.md` ADR table — amend there separately.**

- [2026-05-21] **T/P prefix swap deployed (ADR-025; Constantia task `T-005`).** `T`=Task, `P`=Proposal — the letters were backwards. Telos-side impact: `nextProposalId` now mints `P-NNN`, `nextTaskId` mints `T-NNN`; `grade_task` validates `^T-`, `accept_proposal` validates `^P-`; all WORK/LIFE/LEARN tick prompts + `CLAUDE.local.md` operating contracts swapped (nanoclaw `f8a31c0`); Constantia pre/post-commit hooks + a dated swap legend in Constantia `CLAUDE.md` (`59775be`). Deployed live via a Mini freeze→push→pull→resume cut-over — no container rebuild (`shared/telos-tools/` is bind-mounted RO, `groups/` prompts read live). Dated logs/evidence/archive NOT rewritten (immutable-history rule); pre-swap IDs keep old meaning per the legend. Two recovered mini-local prompt refinements (always-report rule, 기억나무 anchor, midday closing questions) folded in (`2eba7ef`). Live confirmation pending: the next ID-minting tick should write `P-012` (proposal) / `T-006` (task). Full chronicle in `guya/STATUS.md` + `docs/2026-05-21-tp-swap-migration.md`.
