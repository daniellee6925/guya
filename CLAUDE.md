...
| 024 | Docker bind-mount breaks container-side `git rebase`; host-side `constantia-sync` daemon owns push ([details](docs/adrs/adr-024-constantia-sync-daemon.md)) | Container commits via `commitOnly(message, paths)` on bind mount; host daemon polls every 5s and pushes via native APFS git. Heartbeat in `.git/sync-status.json` surfaces silent-rot risk. **Amended 2026-06-08:** daemon also idle-pulls (throttled `fetch + ff-only`) so an idle Mini tracks laptop pushes, and pins the repo deploy key (`GIT_SSH_COMMAND`+`IdentitiesOnly`) instead of silently falling back to the host identity |
| 025 | T/P prefix swap — `T`=Task, `P`=Proposal (was backwards); dated history left immutable with a legend, not rewritten ([plan](docs/2026-05-21-tp-swap-migration.md)) | Spans 3 repos (constantia data+hooks, nanoclaw tool code, guya docs) so atomicity is per-repo, not one commit. Cut-over requires freezing Mini ticks + sync daemon and rebuilding the container — new prefix logic against old-layout data silently renumbers/rejects. Honors Daniel's append-only preference: only live state migrated; `log/`+`evidence/`+`archive/` carry pre-swap meaning per the [[feedback_append_only_migrations]] rule |
| 028 | ~~Plugin runtime install is a symlink to the source tree~~ — **REVERTED same day, 2026-08-03.** Install stays a real copy at `cache/guya/guya/0.1.0`, refreshed by `scripts/sync-plugin.sh` on post-commit | **The symlink experiment broke the plugin.** Hand-editing `installed_plugins.json` to point at a new `0.2.0` symlink caused Claude Code to remove the entire `cache/guya/` tree — both the symlink and the `0.1.0` rollback copy — leaving `installPath` dangling. A missing plugin means no hooks, which means the review gate silently ceases to exist: the exact failure class the change was meant to prevent. Restored by rebuilding `0.1.0` from source via rsync and pointing the config back; verified with `hooks-smoke` (8/8) plus a full gate replay. **Lesson: do not hand-edit Claude Code's internal install state.** The copy-based install has worked since 2026-04-09; its real cost is only that mid-session hook edits need a manual copy to take effect, which is an annoyance, not a risk. The `-type d -o -type l` fixes from the follow-up commit are kept — they are correct either way and cost nothing |
| ~~028a~~ | Superseded detail, kept for the record: the symlink was `~/.claude/plugins/cache/guya/guya/0.2.0` → `guya-plugin` | Claude Code ran a real copy at `cache/.../0.1.0`. `scripts/sync-plugin.sh` refreshed it from a post-commit hook, but **only on commit** — so mid-session hook edits had no runtime effect until a commit landed, and had to be copied by hand. That is the ADR-011/012/013 silent-rot shape again: a trusted mechanism whose failure mode is "nothing happens, nobody notices." Symlinking makes source and runtime the same bytes, so the sync step can't be forgotten because it no longer exists. Verified through the new path with `hooks-smoke` (8/8) plus a live gate replay. Old `0.1.0` dir kept as rollback; `installed_plugins.json` backed up. **Caught immediately after landing:** sync-plugin.sh selected its target with `find -type d`, which does not match symlinks — so it kept syncing to the now-unread `0.1.0` while the runtime read `0.2.0`. Fixed to match `-type d -o -type l` and to no-op (exit 0) when the install is a symlink; the script stays because a `/plugin` reinstall can replace the symlink with a copy, at which point syncing silently starts mattering again. **The same `-type d` bug existed in two more places** — the post-commit hook and its `guya-setup` template — so the scribe resolved to the stale copy too. It had been correct only by accident (sync ran first each commit); no-oping the sync would have frozen it. All three fixed, plus a dangling-symlink guard that fails loud instead of reporting "already live" while the plugin cannot load. If a hook edit ever seems inert, `readlink` the install path first |
| 027 | Pre-commit gate is a three-pass chain: `/guya-review` → `/guya-deep-review` → `/guya-optimize`, order-enforced (`REQUIRED_CHAIN` in `hooks/review-evidence.mjs`) | Review and deep-review both hunt for defects — things that are *wrong*. Neither asks whether correct code should exist in that shape at all, so every commit shipped without the simplification question ever being asked. Optimize runs last because simplification advice is only meaningful once the code is known to work. It stays report-only: recording the pass means "the trade-offs were surfaced and consciously accepted", not "fixes applied". The chain is data-driven so adding or reordering a pass is one array edit; presence and ordering are checked interleaved so a reset chain names the violated link, not the last missing step |
| 026 | One active session per channel + recurring-schedule carry-over on supersede (nanoclaw `6f432a6`) ([details](telos%20context/STATUS.md) — Tests & Observations + Decisions & Notes, 2026-06-11) | Recurring ticks live as rows in a per-session `inbound.db`, not central config. A session rotation (ADR-018 prompt redeploy spawns a fresh session) stranded the WORK schedule in a now-closed session; the host sweep is active-sessions-only, so ticks silently stopped firing for ~2 days while the channel still answered live messages. Retire sessions only through `supersedeSession` (migrate schedule → close; throw rather than strand on a missing target) + boot-time `reconcileChannelSessions` heal that collapses duplicate-active channels onto the newest session. Same silent-rot meta-pattern as ADR-011/012/013 |
