...
| 028 | Plugin runtime install is a **symlink to the source tree** (`~/.claude/plugins/cache/guya/guya/0.2.0` → `guya-plugin`), not a copy | Claude Code ran a real copy at `cache/.../0.1.0`, installed 2026-04-09 and never refreshed, so every hook edit needed a manual copy into the cache or it silently had no runtime effect. That is the ADR-011/012/013 silent-rot shape again: a trusted mechanism whose failure mode is "nothing happens, nobody notices." Symlinking makes source and runtime the same bytes — the sync step can't be forgotten because it no longer exists. Verified through the new path with `hooks-smoke` (8/8) plus a live gate replay. Old `0.1.0` dir kept as rollback; `installed_plugins.json` backed up. Watch: a `/plugin` reinstall could replace the symlink with a copy and silently restore the old problem — `readlink` it first if a hook edit ever seems inert |
