...
| `guya-audit/` | guya-audit | Whole-repo rot audit → GitHub issues. Deterministic mechanical pass (`scripts/scan-standards.mjs`) plus a **12-angle judgment fan-out** — one agent per orthogonal failure mode, ordered by expected yield on agent-built code (duplication & convention drift, silent failure, growth & cost of change, dead weight, doc drift, error paths, contracts, state, test integrity, data integrity, concurrency, trust), sharded and deduped by fingerprint. Files via `/guya-issue` batch mode. Built for unattended runs; the contract is *same code in, same report out*. Files only, never fixes. |
