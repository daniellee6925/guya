...
| `guya-audit/` | guya-audit | Whole-repo rot audit → GitHub issues. Deterministic mechanical pass (`scripts/scan-standards.mjs`) plus an **11-angle judgment fan-out** — one agent per orthogonal failure mode (duplication, dead weight, contracts, error paths, state, data integrity, concurrency, trust, scale, test integrity, doc drift), sharded and deduped by fingerprint. Files via `/guya-issue` batch mode. Built for unattended runs; the contract is *same code in, same report out*. Files only, never fixes. |
