...
| `guya-audit/` | guya-audit | Whole-repo rot audit → GitHub issues. Deterministic mechanical pass (`scripts/scan-standards.mjs`) plus fixed-order judgment lenses, deduped by fingerprint. Built for unattended runs; the contract is *same code in, same report out*. Files only, never fixes. |
