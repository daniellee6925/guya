...
**Visibility-fix exports (guya#3, 2026-05-21).** Exports `computeReflectionNudge(cwd, opts)` (pure: returns the bare `"📝 N reflections accumulated…"` line or `null`) and `formatNudgeDirective(nudge)` (pure: wraps the bare nudge with a "🔔 SHOW DANIEL FIRST — he cannot see this block" imperative so Guya relays it in its first reply). `main()` sets the directive-wrapped string as a section in `additionalContext` AND sets the bare nudge as the top-level `systemMessage`. The `additionalContext` directive is the **load-bearing** path — `systemMessage` is silently dropped for plugin-sourced SessionStart hooks in Claude Code 2.1.x, so it's best-effort native-CLI insurance only. Same value used for both surfaces (no drift risk). See ARCHITECTURE Decision Log "Visibility-fix pattern" entry — applies to any future hook that needs user-visible output, not just the nudge.
...
- **`$VAR` add-token false-block (fixed 2026-06-09, guya `2f16faa`)**: The review gate repeatedly blocked Constantia reflection-log commits with "Review required for 1 non-exempt files" even though the user-wide config exempts `*.md`. The "1 non-exempt file" was a phantom: `getStagedFiles` scrapes `git add` arguments from the *raw, pre-shell* command string (a TOCTOU guard for combined `git add … && git commit`), and `/guya-reflect`'s `git add "$LOG" && git commit` fed the literal token `$LOG` into the staged set. `$LOG` has no extension → matched no `reviewExempt` entry → counted as non-exempt → blocked. This is a fail-*closed* bug (annoying, not dangerous) that misfires on any shell variable, glob, or substitution in an add argument. Fixed with `isShellExpansion`, which drops tokens containing `$ \` * ? [ ] { }` before exemption-matching; the `git diff --cached` source still captures the real staged state. Not a silent-rot case — it surfaced loudly and immediately — but the same lesson applies: a "predict the staged files" heuristic that reads pre-expansion text must not treat shell metacharacters as literal paths. Constantia separately opted out via `config.disabled` (it's prose/data), but the fix matters for every repo that commits via a variable.
