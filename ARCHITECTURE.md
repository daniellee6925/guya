...
**Three-pass chain (ADR-027):** The gate requires `/guya-review` → `/guya-deep-review` → `/guya-optimize`, in that order, inside the same age window. `REQUIRED_CHAIN` in `review-evidence.mjs` is the single source of truth for the contract — step order, recording skill, and every block message derive from it, so adding or reordering a pass is one array edit. Presence and ordering are validated interleaved per link, so a chain reset at an early link reports *that* violation rather than the last missing step. The tree-identity check pins the last chain step.

**Auto-evidence recording (working as of 2026-04-27, ADR-013):** When `/guya-review`, `/guya-deep-review`, or `/guya-optimize` is invoked via `PreToolUse:Skill`, `guya-pre-commit-review.mjs` calls `appendStep()` to write a row into `review-evidence.jsonl` automatically — no manual Step 0 in the SKILL.md required. This had been silently no-op'ing since the symlinked plugin install became the dispatch path, because every hook gated `main()` with `fileURLToPath(import.meta.url) === process.argv[1]` and Node 24 resolves the LHS to the realpath while the RHS keeps the symlink path. Five hook scripts were patched to wrap both sides in `realpathSync()`: `guya-pre-commit-review`, `guya-correction-detect`, `guya-post-commit-scribe`, `guya-session-start`, `guya-session-end`.
...
- [2026-06-09] **Review gate: per-repo `disabled` opt-out + shell-expansion add-token fix (guya `2f16faa`, constantia `8a87acd`).** The gate now honors `"disabled": true` in a repo's `.guya/pre-commit-config.json` (strict `=== true`, short-circuits before file parsing) so pure prose/data repos (Constantia) can opt out without touching the user-wide default that keeps code repos gated. Separately, `getStagedFiles`'s TOCTOU arg-scraper no longer treats unexpanded shell tokens (`$VAR`, globs, substitution) as literal paths — `isShellExpansion` drops them — fixing a false "non-exempt file" block on every variable-based commit (`git add "$LOG" && git commit`, e.g. `/guya-reflect`). See `guya-plugin/hooks/CLAUDE.md` Regression History.
